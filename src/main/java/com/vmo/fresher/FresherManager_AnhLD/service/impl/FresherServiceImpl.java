...
public class FresherServiceImpl implements FresherService {
    ...
    public ResponseObject findByName(String name) {
        ...
        else return  new ResponseObject("found","Found "+foundFresher.size()+"freshers contain \"" + name+"\"",foundFresher.stream().collect(Collectors.toList()));

    }
    @Override
    public ResponseObject findByEmail(String email) {
        List<Fresher> foundFresher = fresherRepository.findFresherByEmailContaining(email);
        if(foundFresher.isEmpty()){ throw
                 new EntityNotFoundException(ApiErrorDetail.builder()
                        .message("Your search '"+ email +"' did not match any documents.")
                        .entityName("Fresher")
                        .fieldName("email")
                        .fieldValue(email)
                        .httpStatus(HttpStatus.NOT_FOUND)
        ...
        else return  new ResponseObject("found","Found "+foundFresher.size()+"freshers contain \"" + email+"\"",foundFresher.stream().collect(Collectors.toList()));
    }
...
//                });
        return new ResponseObject("Updated","Update Fresher succesfully",updatedFresher);
    }
